...
- **튐 감지** — 인접 샘플 차이의 MAD 기반 이상치. 상수 채널이면 MAD 가 0 이라
  평균 편차로 떨어진다
...
- `wave` CHOP (sin) → `check_loop` 가 루프 **가능** 판정
- 같은 파형에 `shift` 를 걸어 끊기게 한 것 → 루프 **불가** 판정
- 프레임 60 에 일부러 튐을 넣은 채널 → `find_spikes` 가 **프레임 60 을 집는다**
- `constant` CHOP → `channel_stats` 의 정지 구간이 전 구간
- `lag` 필터 전후 → 표준편차가 줄어든다
