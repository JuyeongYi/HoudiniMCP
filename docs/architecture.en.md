...
├─ houdini_mcp_base          context-independent tools (102)
├─ houdini_mcp_sop           SOP (30)
├─ houdini_mcp_lop           LOP / USD stage (22)
├─ houdini_mcp_mat           materials (20)
├─ houdini_mcp_hda           HDAs (19)
├─ houdini_mcp_rig           KineFX / APEX rigging (19)
├─ houdini_mcp_dop           DOP common (18)
│  └─ houdini_mcp_dop_rbd    RBD diagnostics (2)
├─ houdini_mcp_chop          CHOP / channels (17)
├─ houdini_mcp_render        rendering (13)
├─ houdini_mcp_io            export / import (8)
├─ houdini_mcp_vex           VEX validation and wrangles (8)
├─ houdini_mcp_cop           COP (Copernicus) preview (2)
└─ houdini_mcp_example       minimal pack example (1)
```

Numbers are tool counts (281 in total, 2026-09-13). `houdini_mcp_base` is the exception:
it holds everything used regardless of context and is split into modules rather than packs
(`info`, `edit`, `parms`, `geometry`, `viewport`, `cache`, `scene`, `deps`, and others — 20
modules). A specialised sub-pack (`dop_rbd`) lists its parent pack (`dop`) in `requires`.

### Files of one pack

```
houdini_mcp_<domain>.json                                    package definition (hpath, env, requires)
...
houdini_mcp_<domain>/README.md                               tool list (generated by scripts/gen_pack_readmes.py)
```
...
sequenceDiagram
    ...
    H->>H: process *.json in HOUDINI_PACKAGE_DIR (hpath, env, requires)
...
1. Copy `houdini_mcp_example` and rename it to `houdini_mcp_<domain>` (JSON file name,
   directory and Python package).
...
4. Generate the pack README with `python scripts/gen_pack_readmes.py`.
...
sets the repository root as `HOUDINI_PACKAGE_DIR` so every pack loads at once (`-Port`,
`-IsolatePrefs`, `-NoTools`, `-LogDir`, `-ConsoleLog`).
...
| Missing registrations | `@tool` count in code = registry count = MCP `tools/list` count (281 on 2026-09-13) |
| Pack READMEs up to date | `python scripts/gen_pack_readmes.py --check` |
